...
<legend id="titre">Statistiques : Immatriculation</legend>
        ...
        <optgroup>
            ...
            <?php
                $cols = array('sexe'=>'Sexe', 
                              'ville_residence_gabon'=>'Résidence',
                              'date_naissance'=>'Année de naissance',
                              'groupe_sanguin_id'=>'Groupe sanguin'
                              );
    ...
    <a class="btn btn-info" onclick="load_statistique('statistiques.php?param=im')"> Valider </a>
    ...
    foreach($tab_critere as $tab_critere) {
        ...
        if ($critere == "date_naissance") {
            ...
            $req_critere = Doctrine_Query::create()
                                ...
                                ->where('i.date_immat BETWEEN :debutAnnee AND :finAnnee', array(':debutAnnee' => $debutAnnee, ':finAnnee' => $finAnnee))
        ...
        if ($critere == "groupe_sanguin_id") {
            
            $gs = Doctrine_Core::getTable('GroupeSanguin')->find($val_critere);  
            $xLabel[] = utf8_encode($gs->libelle);
